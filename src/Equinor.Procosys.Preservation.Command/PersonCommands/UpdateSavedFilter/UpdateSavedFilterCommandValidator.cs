    ...
    public class UpdateSavedFilterCommandValidator : AbstractValidator<UpdateSavedFilterCommand>
    {
            ...
            IRowVersionValidator rowVersionValidator)
        {
            ...
            RuleFor(command => command)
                ...
                .MustAsync((command, token) => NotExistASavedFilterWithSameTitleForPerson(command.Title, command.SavedFilterId, token))
            ...
            async Task<bool> NotExistASavedFilterWithSameTitleForPerson(string title, int savedFilterId, CancellationToken token)
                => !await savedFilterValidator.ExistsWithSameTitleForPersonInProjectAsync(savedFilterId, title, token);
