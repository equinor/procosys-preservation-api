...
public class ProjectTagAddedEventHandlerTests
{
    ...
    public void Setup()
    {
        ...
        var mockConverter = new Mock<IDomainToIntegrationEventConverter<ChildAddedEvent<Project, Tag>>>();
        mockConverter.Setup(x => x.Convert(It.IsAny<ChildAddedEvent<Project, Tag>>())).ReturnsAsync(new List<IIntegrationEvent> { mockIntegrationEvent.Object });
