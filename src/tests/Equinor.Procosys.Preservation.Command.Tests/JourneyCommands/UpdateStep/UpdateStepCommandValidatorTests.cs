...
namespace Equinor.Procosys.Preservation.Command.Tests.JourneyCommands.UpdateStep
{
    ...
    public class UpdateStepCommandValidatorTests
    {
        ...
        private string _responsibleCode = "COD";
        ...
        public void Validate_ShouldFail_WhenResponsibleNotExists()
        {
            ...
            Assert.IsFalse(result.IsValid);
            Assert.AreEqual(1, result.Errors.Count);
            Assert.IsTrue(result.Errors[0].ErrorMessage.StartsWith("Responsible doesn't exist!"));
        }

        [TestMethod]
        public void Validate_ShouldFail_WhenResponsibleIsVoided()
        {
            _responsibleValidatorMock.Setup(r => r.IsVoidedAsync(_responsibleCode, default)).Returns(Task.FromResult(true));
