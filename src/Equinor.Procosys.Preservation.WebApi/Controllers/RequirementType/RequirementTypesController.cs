...
namespace Equinor.Procosys.Preservation.WebApi.Controllers.RequirementType
{
    ...
    public class RequirementTypesController : ControllerBase
    {
        ...
        {
            var result = await _mediator.Send(new GetAllRequirementTypesQuery(plant, includeVoided));
            return Ok(result);
        }
        ...
        public async Task<ActionResult<RequirementTypeDto>> GetRequirementType(
            ...
            [FromRoute] int id)
        {
            var result = await _mediator.Send(new GetRequirementTypeByIdQuery(plant, id));
            return Ok(result);
        }
    }
}
